              ...
              return (
                <div key={category.slug} className="flex items-center hover:cursor-pointer hover:text-gray-500 transition-all">
                  <input
                    ...
                    onClick={() => setFilters({...filters, categories: [...filters.categories, category.slug]})}
                  />
                  <label htmlFor={category.slug} className="hover:cursor-pointer ml-3 pr-6 text-sm font-medium text-gray-900 whitespace-nowrap">
                    {category.name}
                  </label>
                </div>
              );
