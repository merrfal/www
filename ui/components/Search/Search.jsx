...
export default function Search() {
  ...
  if(search.Visibility) 
    ...
    <div className='fixed inset-0 z-20 p-4 sm:p-6 md:p-20 overflow-hidden'>
      <div onClick={() => dispatch(CloseSearch())} className='fixed inset-0 bg-gray-700 bg-opacity-25 transition-opacity overflow-hidden'></div>
      <div className='mx-auto max-w-xl transform divide-y divide-gray-100 rounded-xl bg-white shadow-2xl ring-1 ring-black ring-opacity-5 transition-all overflow-hidden'>
...

        {
         ...
         <div className='py-14 px-6 text-center text-sm sm:px-14'>
          ...
          <p className='mt-4 font-semibold text-gray-900'>Nuk u gjet asnjë rezultat</p>
          ...
          role='listbox'>
          {
            ...
            <Link href={`/produktet/${product.Slug}`}>
              <li onClick={() => dispatch(CloseSearch())} key={index} className='group flex cursor-default select-none rounded-xl p-3 hover:opacity-75 hover:cursor-pointer'>
