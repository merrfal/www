...
const Create = async (payload, res, req) => {
    try {
        ...
        if (uid) {
            ...
            if (user) {
                if (user.userAdditionalData.isBanned === true || user.userAdditionalData.isBanned === undefined) Response({
                    res,
                    code: 403,
                    success: false,
                    data: null,
                    ...
                    if (validations) {
                        ...
                        const user_products = await Product.find({ 
                          'productData.user': user._id.toString(),
                          'createdAt': { $gte: oneHourAgo }
                        })

                        if (user_products.length >= 5) Response({
                            res,
                            code: 429,
                            success: false,
                            data: null,
                            message: 'Ju nuk mund të postoni më shumë se 5 produkte brenda një ore, ju lutem prisni!',
                        })

                        else {
                            let 
                                city = payload?.productData?.city, 
                                country = payload?.productData?.country,
                                category
